 ...
 * Habito data layer.
 *
 * Every method is async and returns plain domain objects, so the storage
 * behind it can be replaced without touching a single component. Today that
 * storage is the browser; swapping in a FastAPI + PostgreSQL backend means
 * reimplementing this one module against fetch() and nothing else changes.
 *
 * Seed records are loaded once and then persisted, so anything the owner
 * creates, edits or deletes survives a refresh.
 */
